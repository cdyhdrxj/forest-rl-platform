...
namespace Unity.RenderStreaming.Samples
{
    ...
    class SimpleCameraControllerArrows : MonoBehaviour
    {
        class CameraState
        {
            public float yaw;
        ...
        [Tooltip("Rotation Speed for keyboard arrows")]
        [SerializeField]
        private float keyboardRotationSpeed = 100f; // Новая настройка скорости вращения от стрелок
        ...
        // НОВЫЙ МЕТОД: Вращение камеры от стрелок
        void UpdateRotationFromKeyboard()
        {
            Vector2 rotationInput = Vector2.zero;
            
            foreach (var keyboard in listKeyboard)
            {
                // Стрелка вверх - поворот вниз (уменьшаем pitch)
                if (keyboard.upArrowKey.isPressed)
                {
                    rotationInput.y -= keyboardRotationSpeed * Time.deltaTime;
                }
                // Стрелка вниз - поворот вверх (увеличиваем pitch)
                if (keyboard.downArrowKey.isPressed)
                {
                    rotationInput.y += keyboardRotationSpeed * Time.deltaTime;
                }
                // Стрелка влево - поворот влево (уменьшаем yaw)
                if (keyboard.leftArrowKey.isPressed)
                {
                    rotationInput.x -= keyboardRotationSpeed * Time.deltaTime;
                }
                // Стрелка вправо - поворот вправо (увеличиваем yaw)
            ...
            {
                // Для стрелок инвертируем Y, если нужно
                if (invertY)
                {
                    rotationInput.y *= -1;
                }
                
                m_TargetCameraState.yaw += rotationInput.x;
                m_TargetCameraState.pitch += rotationInput.y;
        ...
        void FixedUpdate()
        {
            ...
            // НОВЫЙ КОД: Вращение от стрелок (добавлено здесь)
            UpdateRotationFromKeyboard();

            // Rotation from mouse (оставляем как есть)
            foreach (var mouse in listMouse)
            {
