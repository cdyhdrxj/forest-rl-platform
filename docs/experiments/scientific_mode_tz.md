...
Рабочее техническое задание на внедрение отдельного научного режима в проект `ForestRobotTwin`.

Часть MVP уже реализована. Актуальная пользовательская спецификация текущего конфига, команды запуска и структуры отчета находится в `docs/experiments/scientific_mode.md`. Этот документ остается целевым ТЗ и содержит требования, которые шире текущей реализации.
