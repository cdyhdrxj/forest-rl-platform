...
Этот документ описывает standalone-модуль патрулирования и его модели. Платформенный backend route `discrete/patrol` подключается через `apps/api/dispatcher.py`, а общий жизненный цикл run описан в `docs/runtime/README.md`. Команды ниже полезны для локальной разработки самого patrol-модуля и не заменяют запуск всей платформы через Docker Compose.
